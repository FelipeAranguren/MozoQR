...
export default function OwnerDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.get(`/restaurantes?filters[slug][$eq]=${slug}&populate[productos][populate]=image&populate[mesas]=true&populate[categorias]=true&populate[logo]=true`).catch(() => ({ data: { data: [] } })),
    ])
      .then(([orders, todayOrders, yesterdayOrders, totalOrd, sessions, topProd, restaurantRes]) => {
  ...
  const successScoreData = useMemo(() => {
    return calculateSuccessScore(restaurantMetrics);
  }, [restaurantMetrics]);
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        <Grid item xs={12} sm={6} md={3}>
          <KpiCardEnhanced
            ...
            formatter={money}
            trend={derivedKpis.todayVsYesterday ? {
              value: derivedKpis.todayVsYesterday.percentChange,
              label: 'vs ayer'
            } : undefined}
