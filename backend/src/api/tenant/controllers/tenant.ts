...
const { ValidationError, NotFoundError, ForbiddenError } = errors;

declare const strapi: any;

// --- helper: obtener restaurante + plan ---
async function getRestaurant(idOrSlug: { id?: any; slug?: string }) {
  const q = idOrSlug.id
...
async function createOrder(ctx) {
  ...
  if (!Number.isFinite(Number(table))) throw new ValidationError('Mesa inválida');
  if (!Array.isArray(items) || items.length === 0) throw new ValidationError('Items requeridos');
  ...
  const byId = new Map<number, Product>(prods.map((p: any) => [Number(p.id), { id: Number(p.id), price: Number(p.price) }]));
  let subtotal = 0;
  const saneItems = items.map(it => {
    const pid = Number(it.id || it.productId);
    const qty = Number(it.qty || it.quantity || 0);
    if (!pid || !qty || qty <= 0) throw new ValidationError('qty>0 requerido');
    const p = byId.get(pid);
    if (!p) throw new ValidationError(`Producto ${pid} no disponible`);
  ...
  const mesa = await strapi.entityService.findMany('api::mesa.mesa', {
    ...
    fields: ['id','number'],
    limit: 1,
  }).then(rows => rows?.[0]) ?? await strapi.entityService.create('api::mesa.mesa', {
    data: { number: Number(table), restaurante: restaurantId, publishedAt: new Date() },
  });

  const open = await strapi.entityService.findMany('api::mesa-sesion.mesa-sesion', {
    filters: { restaurante: { id: restaurantId }, mesa: { id: mesa.id }, session_status: 'open' },
    fields: ['id','code'],
    limit: 1,
  }).then(rows => rows?.[0]) ?? await strapi.entityService.create('api::mesa-sesion.mesa-sesion', {
...
async function listOrders(ctx) {
  ...
  const filters: any = { restaurante: { id: restaurantId } };
...
async function updateOrderStatus(ctx) {
  ...
  if (!status) throw new ValidationError('status requerido');
  ...
  if (!pedido?.id) throw new NotFoundError('Pedido no encontrado');
  if (Number(pedido.restaurante?.id || pedido.restaurante) !== Number(restaurantId)) {
...
async function createPaymentMock(ctx) {
  ...
  if (!orderId) throw new ValidationError('orderId requerido');
...
export default { menus, createOrder, listOrders, updateOrderStatus, createPaymentMock };
