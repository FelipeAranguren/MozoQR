...
export default {
    ...
    async today(ctx: any) {
        ...
        const salesToday = (paidToday || []).reduce((s: number, r: any) => s + Number(r.total || 0), 0);
    ...
    async exportCsv(ctx: any) {
        ...
        const filters: any = { restaurante: restauranteId, createdAt: { $gte: from, $lte: to } };
        ...
        for (const r of rows || []) {
            const mesa = r.mesa_sesion?.mesa?.number
                ...
                || '';
            const itemsStr = (r.items || []).map((it: any) => {
